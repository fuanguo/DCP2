function DCP_run(opt)
    ...
    fid=fopen([logPath filesep 'logs.txt'],'w');
...
%             pipeline.(jobnameMove).files_out=[opt.inputFile filesep subFile(i).name filesep];
              try
               ...
               try
                  ...
                  fprintf(fid,[subFile(i).name '''s convertion is done\n']);
              catch
...
%     end
    ...
    %         DCP_merge_matrix(opt);
        try
            DCP_merge_matrix(opt);
            fprintf(fid,[subFile(i).name '''s matrix merging is done\n']);
        catch
            fprintf(fid,[subFile(i).name '''s matrix merging is failed\n']);
        end
    end
    fclose(fid);
end